...
if len(sys.argv) != 2:
    print("Usage: gem5 l1_only_x86.py <binary>")
    sys.exit(1)

binary = sys.argv[1]

# -------------------------
# Cache definitions (L1 only)
# -------------------------
class L1ICache(Cache):
    size = '16kB'
    assoc = 2                 # set to 1 for direct-mapped
    tag_latency = 2
    data_latency = 2
    response_latency = 2
    mshrs = 4
...
class L1DCache(Cache):
    size = '64kB'
    assoc = 8                 # set to 1 for direct-mapped
    tag_latency = 2
    data_latency = 2
    response_latency = 2
    mshrs = 4
...
# Connect L1s directly to memory bus (no L2/L3)
...
# System port (for initialization)
system.system_port = system.membus.cpu_side_ports

# Workload + process
try:
    # Works on newer gem5; fallback below if not available
...
print("Configuration: L1-only (no L2/L3)")
