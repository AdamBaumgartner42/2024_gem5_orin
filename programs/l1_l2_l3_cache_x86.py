    ...
    try:
        ...
        if ratios:
            ...
            if 'l3cache_hit_ratio' in ratios:
                ...
                print(f"L3 Cache Miss Ratio:      {ratios['l3cache_miss_ratio']:.2f}%")
                print(f"L3 Cache Hits:            {int(ratios['l3cache_hits'])}")
                print(f"L3 Cache Misses:          {int(ratios['l3cache_misses'])}")
                print(f"L3 Cache Total Accesses:  {int(ratios['l3cache_total_accesses'])}")
                print()
            ...
            if 'icache_hits' in ratios and 'dcache_hits' in ratios:
                ...
                l1_total_accesses = ratios['icache_total_accesses'] + ratios['dcache_total_accesses']
            ...
            if l1_total_accesses > 0:
                l1_overall_hit_ratio = (l1_total_hits / l1_total_accesses) * 100
                l1_overall_miss_ratio = 100 - l1_overall_hit_ratio
                
                print(f"L1 Cache Level Performance:")
                print(f"  Overall L1 Hit Ratio:     {l1_overall_hit_ratio:.2f}%")
                print(f"  Overall L1 Miss Ratio:    {l1_overall_miss_ratio:.2f}%")
                print(f"  Total L1 Accesses:       {int(l1_total_accesses)}")
                print(f"  Total L1 Hits:           {int(l1_total_hits)}")
                print(f"  Total L1 Misses:         {int(l1_total_accesses - l1_total_hits)}")
                print()
            ...
            if l2_total_accesses > 0:
                ...
                print(f"  Total L2 Misses:         {int(l2_total_accesses - l2_total_hits)}")
                print()
            ...
            if l3_total_accesses > 0:
                ...
                print(f"  Total L3 Misses:         {int(l3_total_accesses - l3_total_hits)}")
                print()
            ...
            if l1_total_accesses > 0:
                ...
                if l2_total_accesses > 0:
                    l1_efficiency = ((l1_total_accesses - l2_total_accesses) / l1_total_accesses) * 100
                    l1_traffic_reduction = l1_total_accesses - l2_total_accesses
                    print(f"L1 Cache Efficiency:      {l1_efficiency:.2f}%")
                    print(f"L1 Traffic Reduction:     {int(l1_traffic_reduction)} requests")
                    print(f"L1 → L2 Traffic:          {int(l2_total_accesses)} requests")
                    print()
                
                # L2 efficiency (how much L2 reduces traffic to L3)
                if l2_total_accesses > 0 and l3_total_accesses > 0:
                    l2_efficiency = ((l2_total_accesses - l3_total_accesses) / l2_total_accesses) * 100
                    l2_traffic_reduction = l2_total_accesses - l3_total_accesses
                    print(f"L2 Cache Efficiency:      {l2_efficiency:.2f}%")
                    print(f"L2 Traffic Reduction:     {int(l2_traffic_reduction)} requests")
                    print(f"L2 → L3 Traffic:          {int(l3_total_accesses)} requests")
                    print()
                elif l2_total_accesses > 0:
                    print(f"L2 Cache Efficiency:      100.00% (no L3 traffic)")
                    print(f"L2 Traffic Reduction:     {int(l2_total_accesses)} requests")
                    print(f"L2 → Memory Traffic:      {int(l2_total_accesses - l2_total_hits)} requests")
                    print()
                
                # L3 efficiency (how much L3 reduces traffic to memory)
                if l3_total_accesses > 0:
                    memory_accesses = l3_total_accesses - l3_total_hits
                    l3_efficiency = (l3_total_hits / l3_total_accesses) * 100 if l3_total_accesses > 0 else 0
                    print(f"L3 Cache Efficiency:      {l3_efficiency:.2f}%")
                    print(f"L3 Traffic Reduction:     {int(l3_total_hits)} requests")
                    print(f"L3 → Memory Traffic:      {int(memory_accesses)} requests")
                    print()
                
                # Overall system efficiency
                if l3_total_accesses > 0:
                    memory_requests = l3_total_accesses - l3_total_hits
                    overall_efficiency = ((l1_total_accesses - memory_requests) / l1_total_accesses) * 100
                elif l2_total_accesses > 0:
                    memory_requests = l2_total_accesses - l2_total_hits
                    overall_efficiency = ((l1_total_accesses - memory_requests) / l1_total_accesses) * 100
                else:
                    memory_requests = l1_total_accesses - l1_total_hits
                    overall_efficiency = (l1_total_hits / l1_total_accesses) * 100
            ...
            if l1_total_accesses > 0:
                print(f"CPU → L1:     {int(l1_total_accesses):>8} requests (100.0%)")
                
                if l2_total_accesses > 0:
                    l1_miss_percent = (l2_total_accesses / l1_total_accesses) * 100
                    print(f"L1 → L2:      {int(l2_total_accesses):>8} requests ({l1_miss_percent:.1f}%)")
                    
                    if l3_total_accesses > 0:
                        l2_miss_percent = (l3_total_accesses / l1_total_accesses) * 100
                        print(f"L2 → L3:      {int(l3_total_accesses):>8} requests ({l2_miss_percent:.1f}%)")
                        
                        memory_requests = l3_total_accesses - l3_total_hits
                        memory_percent = (memory_requests / l1_total_accesses) * 100
                        print(f"L3 → Memory:  {int(memory_requests):>8} requests ({memory_percent:.1f}%)")
                    else:
                        memory_requests = l2_total_accesses - l2_total_hits
                        memory_percent = (memory_requests / l1_total_accesses) * 100
                        print(f"L2 → Memory:  {int(memory_requests):>8} requests ({memory_percent:.1f}%)")
                else:
                    memory_requests = l1_total_accesses - l1_total_hits
                    memory_percent = (memory_requests / l1_total_accesses) * 100
                    print(f"L1 → Memory:  {int(memory_requests):>8} requests ({memory_percent:.1f}%)")
                print()
            ...
            if l1_total_accesses > 0:
                ...
                # L1 hits
                l1_hit_contribution = (l1_total_hits / l1_total_accesses) * l1_latency
                avg_access_time += l1_hit_contribution
                
                if l2_total_accesses > 0:
                    # L2 hits (L1 miss + L2 hit)
                    l2_hit_contribution = (l2_total_hits / l1_total_accesses) * (l1_latency + l2_latency)
                    avg_access_time += l2_hit_contribution
                    
                    if l3_total_accesses > 0:
                        # L3 hits (L1 miss + L2 miss + L3 hit)
                        l3_hit_contribution = (l3_total_hits / l1_total_accesses) * (l1_latency + l2_latency + l3_latency)
                        avg_access_time += l3_hit_contribution
                        
                        # Memory accesses (L1 + L2 + L3 miss)
                        memory_requests = l3_total_accesses - l3_total_hits
                        memory_contribution = (memory_requests / l1_total_accesses) * (l1_latency + l2_latency + l3_latency + memory_latency)
                        avg_access_time += memory_contribution
                    else:
                        # Memory accesses (L1 + L2 miss)
                        memory_requests = l2_total_accesses - l2_total_hits
                        memory_contribution = (memory_requests / l1_total_accesses) * (l1_latency + l2_latency + memory_latency)
                        avg_access_time += memory_contribution
                else:
                    # Memory accesses (L1 miss only)
                    memory_requests = l1_total_accesses - l1_total_hits
                    memory_contribution = (memory_requests / l1_total_accesses) * (l1_latency + memory_latency)
                    avg_access_time += memory_contribution
                
                # Compare with no-cache scenario
                no_cache_time = memory_latency
                speedup = no_cache_time / avg_access_time if avg_access_time > 0 else 1
                
                print(f"Average Memory Access Time: {avg_access_time:.2f} cycles")
                ...
                print(f"  L1 Hit Contribution:      {l1_hit_contribution:.2f} cycles ({(l1_hit_contribution/avg_access_time)*100:.1f}%)")
                if l2_total_accesses > 0:
                    print(f"  L2 Hit Contribution:      {l2_hit_contribution:.2f} cycles ({(l2_hit_contribution/avg_access_time)*100:.1f}%)")
                if l3_total_accesses > 0:
                    print(f"  L3 Hit Contribution:      {l3_hit_contribution:.2f} cycles ({(l3_hit_contribution/avg_access_time)*100:.1f}%)")
                print(f"  Memory Access Contribution: {memory_contribution:.2f} cycles ({(memory_contribution/avg_access_time)*100:.1f}%)")
